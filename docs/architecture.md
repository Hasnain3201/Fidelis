...
- `apps/api` (FastAPI) owns validation, role checks, claim enforcement, and business workflows.
...
- `POST /api/v1/venues/mine`     — create venue + auto-claim (role: venue)
- `PATCH /api/v1/venues/mine`    — update managed venue (role: venue + approved claim)
- `POST /api/v1/venues/events`   — publish event (role: venue + approved claim)
- `POST /api/v1/artists/mine`    — create artist + auto-claim (role: artist)
- `PATCH /api/v1/artists/mine`   — update managed artist (role: artist + approved claim)
...
- **Claims-based ownership**: venues and artists are managed through approved claims rather than `owner_id` foreign keys. This allows entities to exist before account owners are known.
...
- Venue/artist creation uses the admin client (service role) to insert the entity and auto-approve the creator's claim.
