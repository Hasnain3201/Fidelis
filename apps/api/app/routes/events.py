...
def build_event_query(
    ...
    select_clause = (
        f"id,title,start_time,category,zip_code,is_promoted,cover_image_url,"
        f"{venues_join}(name, city, state, zip_code)"
    )
...
def search_events(
    ...
    items = [
        EventSummary(
            ...
            start_time=row["start_time"],
            category=row["category"],
            zip_code=row.get("zip_code") or (row.get("venues") or {}).get("zip_code"),
            is_promoted=bool(row.get("is_promoted", False)),
            cover_image_url=row.get("cover_image_url"),
        )
        for row in rows
